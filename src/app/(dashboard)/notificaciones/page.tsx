...
type NotifItem = InquiryNotif | ReminderNotif
...
export default function NotificacionesPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'inquiries' | 'reminders'>('all')
  ...
  const fetchAll = async () => {
    setLoading(true)
    await Promise.all([fetchInquiries(), fetchReminders()])
    setLoading(false)
  }
  ...
  const fetchReminders = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setReminders(
          data.map((a: { id: string; title: string; type: string; date: string; done: boolean; contact?: { id: string; name: string } | null; inquiry?: { id: string; name: string } | null }) => ({
  ...
  const allItems: NotifItem[] = [
    ...(filter !== 'reminders' ? inquiries : []),
    ...(filter !== 'inquiries' ? reminders : []),
  ].sort((a, b) => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <div className="flex gap-2">
        {(['all', 'inquiries', 'reminders'] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
          ...
          >
            {f === 'all' ? 'Todo' : f === 'inquiries' ? 'Consultas' : 'Recordatorios'}
          </button>
        ))}
      </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant={inquiryStatusVariant[item.status] || 'default'} className="text-xs">
                          {item.status}
                        </Badge>
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                    item.done ? 'bg-green-100' : isPast ? 'bg-red-100' : 'bg-purple-100'
                  }`}>
                    {item.done ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(item.contact || item.inquiry) && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {item.contact && (
                          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">{item.contact.name}</span>
                        )}
                        {item.inquiry && (
                          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">{item.inquiry.name}</span>
                        )}
                      </div>
                    )}
                  </div>
