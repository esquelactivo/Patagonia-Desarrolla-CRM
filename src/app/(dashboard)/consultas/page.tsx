...
export default function ConsultasPage() {
  ...
  // Returns the template message if there's one associated to the inquiry's formulario, else null
  const getQuickWaMsg = (inquiry: { phone?: string | null; adName?: string | null; name: string }) => {
    if (!inquiry.phone || !inquiry.adName) return null
    const tpl = templates.find(t => t.formName === inquiry.adName)
    return tpl ? applyTemplate(tpl, inquiry.name) : null
  }

  const openDetail = (inquiry: Inquiry & { assignedTo?: string | null; assignedUser?: Agent | null }) => {
    setSelectedInquiry(inquiry)
    const tpl = inquiry.adName ? templates.find(t => t.formName === inquiry.adName) : null
    setWaMessage(tpl ? applyTemplate(tpl, inquiry.name) : defaultMessage(inquiry.name))
  }
            ...
            {filtered.map((inquiry) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 pt-1" onClick={(e) => e.stopPropagation()}>
                  {getQuickWaMsg(inquiry) && (
                    <a href={buildWhatsAppUrl(inquiry.phone!, getQuickWaMsg(inquiry)!)} target="_blank" rel="noopener noreferrer"
